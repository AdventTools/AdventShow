...
#   - scp înapoi pe Mac în release/<version>/AdventShow-Setup-<version>.exe
#
# Usage:
...
REMOTE_DIR_FWD="$(echo "${WIN_PROJECT}\\release\\${VERSION}" | tr '\\' '/')"
# Versiunea e în numele fișierului: hangar publică toate versiunile în același
# director, iar electron-updater deduce numele vechi înlocuind versiunea.
EXE_NAME="AdventShow-Setup-${VERSION}.exe"
EXE_LOCAL="release/${VERSION}/${EXE_NAME}"

# 1) The installer itself (required).
