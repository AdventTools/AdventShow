...
scp "${WIN_SSH_OPTS[@]}" -C "$WIN_HOST:${REMOTE_DIR_FWD}/AdventShow-Setup.exe" "$EXE_LOCAL" \
    || { echo "❌ scp Setup.exe înapoi a eșuat"; exit 1; }

# 2) electron-updater metadata: latest.yml (REQUIRED for auto-update) + blockmap
#    (differential-download optimization, best-effort). latest.yml is generated by
#    electron-builder because electron-builder.json5 declares `publish: github`.
scp "${WIN_SSH_OPTS[@]}" -C "$WIN_HOST:${REMOTE_DIR_FWD}/latest.yml" "release/${VERSION}/latest.yml" \
    || { echo "❌ scp latest.yml înapoi a eșuat (necesar pentru auto-update)"; exit 1; }
scp "${WIN_SSH_OPTS[@]}" -C "$WIN_HOST:${REMOTE_DIR_FWD}/AdventShow-Setup.exe.blockmap" "release/${VERSION}/AdventShow-Setup.exe.blockmap" \
    || echo "   ⚠️  blockmap lipsă (non-critic — doar descărcarea diferențială nu va merge)"
